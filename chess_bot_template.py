...
def choose_move(chess):
    # This is where your code goes
    # What you can use:
    # chess.board: the Chess Board as a list of 8 lists of each 8 items; Attention: to check if a field is empty, use `chess.board[i] != ''`
    # chess.turn: the color of the current player ('w' or 'b')
    # chess.get_all_moves(): all moves you can do as dictionary with positions as keys and lists of destinations as values
    # chess.is_draw(): returns True and the reason, if the game ended in a draw
    # chess.is_checkmate(): returns True and the color of the player which won, if the game ended in checkmate
    # chess.convert_field_to_integers(): converts fields, like a2, into integers, like 7 and 3 (indexes for chess.board) 
    # chess.convert_integers_to_field(): converts integers, like 7 and 3 (indexes for chess.board), into fields, like a2

    # You are not allowed to:
    # ... change any variables in chess
    # ... use the function: move

    all_moves = chess.get_all_moves()

    position = choice(all_moves.keys())
    destination = choice(all_moves[position])
    promoto_to = 'Q'

    return position, destination, promoto_to
